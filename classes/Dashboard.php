...
class Dashboard {
    ...
    public function getActiveTableCount() {
        try {
            $query = "SELECT COUNT(*) as active_tables FROM tables WHERE status = 'occupied'";
            $stmt = $this->db->prepare($query);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result['active_tables'];
        } catch (PDOException $e) {
            throw new Exception("Error fetching active tables: " . $e->getMessage());
        }
    }
